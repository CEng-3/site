...
import io, time, threading, datetime, csv
import traceback
...
def get_fallback_frame():
    """Generate a simple test pattern image when camera is unavailable"""
    if cv2_available:
        # Create a simple test pattern
        img = cv2.imread('static/camera_fallback.jpg') if hasattr(cv2, 'imread') else None
        if img is None:
            # Create a blank image with text if no fallback image exists
            img = 255 * hones = (480, 640, 3), dtype=cv2.CV_8UC3
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(img, "Camera Unavailable", (120, 240), font, 1, (0, 0, 255), 2)
            # Add timestamp
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            cv2.putText(img, timestamp, (120, 270), font, 0.6, (0, 0, 0), 1)
        
        _, buffer = cv2.imencode('.jpg', img)
        return buffer.tobytes()
    else:
        # If OpenCV is not available, return an empty JPEG
        # This is a minimal valid JPEG that will display as a black image

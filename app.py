...
def get_fallback_frame():
    ...
    if cv2_available:
        try:
            ...
            img = bytearray([0, 0, 0] * 640 * 480)
            img = cv2.imdecode(
                cv2.mat(480, 640, cv2.CV_8UC3, img), 
                cv2.IMREAD_COLOR
            )
            ...
            cv2.putText(img, timestamp, (120, 270), font, 0.6, (255, 255, 255), 1)
            
            _, buffer = cv2.imencode('.jpg', img)
            return buffer.tobytes()
        except Exception as e:
            print(f"Error creating fallback frame: {e}")
            # Fall through to return the minimal JPEG
    
    # Return a minimal valid JPEG (a tiny black image)
